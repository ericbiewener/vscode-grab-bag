...
import { Node, parseActiveDocument } from './parseText'
...
function findInnerArrowFns(
  node: Node | Node[] | undefined,
  offset: number,
  results: ArrowFunctionExpression[] = [],
) {
  ...
  if (node.type === 'ArrowFunctionExpression') {
    results.push((node as unknown) as ArrowFunctionExpression)
  }
  ...
  if (node.expression) findInnerArrowFns(node.expression.arguments, offset, results)

  // JSX
  if (node.openingElement) findInnerArrowFns(node.openingElement.attributes, offset, results)
  if (_.isObject(node.value)) findInnerArrowFns(node.value.expression, offset, results)
